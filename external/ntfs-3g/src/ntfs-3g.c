...
int main(int argc, char *argv[])
{
    ...
    ntfs_log_info("ntfs-3g start!!!  argv:");
    for(i=0; i<argc; i++){
        ntfs_log_info("argv[%d]:%s", i, argv[i]);
    }
    ...
    ntfs_log_info("parse_mount_options");
    ...
    ntfs_log_info("ntfs_open %s", opts.device);
    ...
    ntfs_log_info("ntfs_xattr_build_mapping");
    ...
    ntfs_log_info("mount_fuse %d", parsed_options);
