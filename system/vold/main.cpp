...
static int process_config(VolumeManager *vm)
{
    ...
    for (i = 0; i < fstab->num_entries; i++) {
        if (fs_mgr_is_voldmanaged(&fstab->recs[i])) {
            SLOGD("process_config fstab->recs label %s blk_device %s mount_point %s", fstab->recs[i].label, fstab->recs[i].blk_device, fstab->recs[i].mount_point);
            ...
            SLOGD("process_config new DirectVolume %s", fstab->recs[i].label);
