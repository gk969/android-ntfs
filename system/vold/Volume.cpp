...
int Volume::mountVol() {
    ...
    for (i = 0; i < n; i++) {
        ...
        SLOGI("try mount %s at %s as NTFS", devicePath, mountPoint);
        if (Ntfs::doMount(devicePath, mountPoint, false, false, AID_EVERYBODY, AID_EVERYBODY, 0000, true)==0) {
            SLOGI("Successfully mount %s as NTFS at %s", devicePath, mountPoint);
        }else{
